...
local function bypassAntiCheat()
    ...
    -- 5. Отключаем скрипты, следящие за телепортом
    for _, v in pairs(game:GetDescendants()) do
...
player:WaitForChild("PlayerGui").ChildRemoved:Connect(function(child)
    if child.Name == "FSOCEITY_MM2" then
        wait(0.5)
        -- Просто перезагружаем скрипт
        loadstring(game:HttpGet("https://raw.githubusercontent.com/твой_ник/твой_репо/main/mm2.lua"))()
    end
end)
...
-- Слайдер спина
local spinSlider = Instance.new("TextLabel")
spinSlider.Size = UDim2.new(0.9, 0, 0, 25)
spinSlider.Position = UDim2.new(0.05, 0, 0.91, 0)
spinSlider.BackgroundTransparency = 1
spinSlider.Text = "Скорость спина: " .. spinSpeed
spinSlider.TextColor3 = Color3.fromRGB(255, 150, 100)
spinSlider.TextScaled = true
spinSlider.Font = Enum.Font.Gotham
spinSlider.Parent = frame
...
status.Text = "✅ Античит обойдён | F1 - STOP"
...
print("[FSOCEITY] 🎯 Аимбот | 👁 Wallhack | 🐇 Bhop | ✨ Шейдеры | 🌀 Спин")
